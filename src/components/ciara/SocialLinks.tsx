...
const items = [
  { href: siteConfig.socials.soundcloud, label: "SoundCloud", icon: "▸▸", bg: "bg-cherry text-cream", rot: "-3deg" },
  { href: siteConfig.socials.audius, label: "Audius", icon: "♪", bg: "bg-toxic text-noir", rot: "2deg" },
  { href: siteConfig.socials.instagram, label: "Instagram / Threads", icon: "◉", bg: "bg-cream text-noir", rot: "-2deg" },
  { href: siteConfig.socials.tiktok, label: "TikTok", icon: "✦", bg: "bg-noir text-cream border border-cream", rot: "3deg" },
];
        ...
        {items.map((it, i) => (
          ...
          >
            <div className={`aspect-square ${it.bg} flex flex-col items-center justify-center clip-notch shadow-[6px_6px_0_hsl(var(--cherry))] group-hover:shadow-[8px_8px_0_hsl(var(--toxic))] transition-shadow`}>
              <div className="text-4xl font-display">{it.icon}</div>
